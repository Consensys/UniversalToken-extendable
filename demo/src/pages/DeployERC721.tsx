...
import { AllowExtension__factory, BlockExtension__factory, CertificateValidatorExtension__factory, ERC721Logic__factory, ERC721__factory, HoldExtension__factory, PauseExtension__factory } from '../ethers-contracts';
...
export default function DeployERC721() {
    ...
    const factory = new ERC721__factory(provider?.getSigner());
    const logicFactory = new ERC721Logic__factory(provider?.getSigner());
    ...
    const deployContract = async () => {
        try {
            ...
            const token = await factory.deploy(name, symbol, allowMint, allowBurn, ethers.utils.parseUnits(maxSupply.toString()), owner, logicAddress);
            await token.deployed();

            if (allowExtension !== '') {
                await token.registerExtension(allowExtension);
            }

            if (blockExtension !== '') {
                await token.registerExtension(blockExtension);
            }

            if (certificateValidatorExtension !== '') {
                await token.registerExtension(certificateValidatorExtension);
            }

            if (holdExtension !== '') {
                await token.registerExtension(holdExtension);
            }

            if (pauseExtension !== '') {
                await token.registerExtension(pauseExtension);
            }
    ...
    return (
        <Container>
            ...
            {alertElement}
            <h1>Deploy ERC721 Extendable Token</h1>
